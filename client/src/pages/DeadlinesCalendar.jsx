...
// Ordered by "how urgently does this need action" - closing/joining windows
// first, since those are the ones a runner could actually miss.
const GROUP_ORDER = [
  "lottery_close",
  "general_entry_close",
  "waitlist_open",
...
export function DeadlinesCalendar() {
  ...
  const upcomingEvents = races
    .flatMap((race) =>
      (race.registrationEvents || [])
        .filter((e) => e.date)
        .map((e) => ({ ...e, raceName: race.name, raceSlug: race.slug, officialUrl: race.officialUrl }))
    )
  ...
  const grouped = GROUP_ORDER.map((type) => ({
    ...
    events: upcomingEvents.filter((e) => e.type === type),
    ...
    <div style={{ padding: "24px 32px", maxWidth: 800, margin: "0 auto" }}>
      ...
      {upcomingEvents.length === 0 && (
        <p style={{ color: "var(--text-secondary)" }}>
          No upcoming dated events yet — research a few races from the dashboard to populate this page.
        </p>
      )}
