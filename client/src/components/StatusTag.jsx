const VARIANT_BY_CONFIDENCE = {
  not_yet_researched: {
    ...
    className: "status-tag--following",
  },
...
export function ConfidenceTag({ confidence }) {
  const variant =
    VARIANT_BY_CONFIDENCE[confidence] ||
    VARIANT_BY_CONFIDENCE.not_yet_researched;
  return (
    <span className={`status-tag ${variant.className}`}>{variant.label}</span>
  );
}
